  ...
  defaultNavigationOptions: ({ navigation }) => ({
    tabBarIcon: ({ focused, horizontal, tintColor }) => {
      ...
      if (routeName === 'Home') {
        ...
        return <IconComponent1 name={'compass'} size={25} color={tintColor} />;
      }
      else if (routeName === 'AlQuran') {
        // iconName = focused ? 'compass-outline' : 'compass-outline';
        return <IconComponent2 name={'book-open'} size={25} color={tintColor} />;
      }
      else if (routeName === 'Dua') {
        // iconName = focused ? 'compass-outline' : 'compass-outline';
        return <IconComponent3 name={'hand-holding'} size={25} color={tintColor} />;
      }
      else if (routeName === 'More') {
        // iconName = focused ? 'compass-outline' : 'compass-outline';
        return <IconComponent4 name={'dots-three-horizontal'} size={25} color={tintColor} />;
      }
